...
> テーマを実際に設計・実装する場合は、先に [`THEME_IMPLEMENTATION_RULES.md`](THEME_IMPLEMENTATION_RULES.md) を読んでください。実装手順、保護対象、Plainへの退避、静的フォールバック、旧テーマ保存のルールは同文書を正本とします。
...
6. **お着替えテーマは実験の場とする**  
   新しいテーマでは、その時点の先進的なWebデザインや実験的な表現を積極的に試してよい。ただし、実験は見せ方に限定し、本文・意味構造・情報公開の継続性を犠牲にしない。
...
詳細な実装フローと公開前チェックは [`THEME_IMPLEMENTATION_RULES.md`](THEME_IMPLEMENTATION_RULES.md) を正本とする。概要は次のとおり。

1. Figma等で新デザインを作る。
2. 新しい `○○Layout.astro` とテーマCSSを**新規追加**する。
3. Markdown本文を一切変更せず、新テーマで全内容が読めることを確認する。
4. モバイル、キーボード操作、文字拡大、リンク、見出し階層を確認する。
5. Plainと過去テーマが壊れていないことを確認する。
6. Preview環境で確認後、テーマ登録と既定値を変更する。
7. 公開後に問題があれば `plain` へ即時退避する。
...
- JavaScriptを情報表示の必須条件にしない
- 既存テーマの延長に限定せず、その時点の先進的・実験的な表現を検討する
- Plainと静的フォールバックは新テーマに合わせて再設計しない
